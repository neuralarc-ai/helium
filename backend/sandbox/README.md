...
1. Change the `image` parameter in `docker-compose.yml` (that defines the image name `kortix/he2`)
...
1. Update the version number in `docker-compose.yml` (e.g., from `he2` to `he3`)
2. Build the new image: `docker compose build`
3. Push the new version: `docker push kortix/he2`
