-- IMMEDIATE FIX for global knowledge base context issue
...
SELECT 'Function fix verification:' as info;
SELECT 
    CASE 
        WHEN pg_get_functiondef(oid) LIKE '%account_id = user_account_id OR account_id = thread_account_id::VARCHAR(255)%' 
        THEN '✅ SUCCESS: Function has been updated with the OR condition'
        ELSE '❌ FAILED: Function was not updated properly'
    END as fix_status
FROM pg_proc 
WHERE proname = 'get_combined_knowledge_base_context'
AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public'); 