...
class ToolkitService:
    ...
    async def get_toolkit(self, toolkit_slug: str) -> Optional[Any]:
        """
        Get toolkit details by slug.
        
        Args:
            toolkit_slug: The slug of the toolkit
            
        Returns:
            The toolkit object if found, None otherwise
        """
        try:
            logger.debug(f"Fetching toolkit details for: {toolkit_slug}")
            toolkits_response = await self.list_toolkits(limit=1, search=toolkit_slug)
            
            if not toolkits_response or not toolkits_response.get('items'):
                logger.warning(f"Toolkit {toolkit_slug} not found")
                return None
                
            # Find the toolkit with matching slug
            for toolkit in toolkits_response['items']:
                if hasattr(toolkit, 'slug') and toolkit.slug == toolkit_slug:
                    return toolkit
                elif hasattr(toolkit, '__dict__') and toolkit.__dict__.get('slug') == toolkit_slug:
                    return toolkit
                    
            logger.warning(f"Toolkit {toolkit_slug} not found in results")
            return None
            
        except Exception as e:
            logger.error(f"Error getting toolkit {toolkit_slug}: {e}", exc_info=True)
            return None
            
    async def get_toolkit_icon(self, toolkit_slug: str) -> Optional[str]:
        """
        Get the icon URL for a specific toolkit.
        
        Args:
            toolkit_slug: The slug of the toolkit
            
        Returns:
            The icon URL if found, None otherwise
        """
        try:
            logger.debug(f"Fetching icon for toolkit: {toolkit_slug}")
            
            # First try to get the toolkit details
            toolkit = await self.get_toolkit(toolkit_slug)
            if not toolkit:
                logger.warning(f"Toolkit {toolkit_slug} not found")
                return None
                
            # The logo URL might be in the toolkit data
            if hasattr(toolkit, 'logo') and toolkit.logo:
                return toolkit.logo
                
            # If not in the main object, check meta
            if hasattr(toolkit, 'meta') and toolkit.meta and hasattr(toolkit.meta, 'logo') and toolkit.meta.logo:
                return toolkit.meta.logo
                
            # If still not found, try to get it from the raw data
            if hasattr(toolkit, '__dict__'):
                toolkit_data = toolkit.__dict__
            elif hasattr(toolkit, '_asdict'):
                toolkit_data = toolkit._asdict()
            else:
                toolkit_data = toolkit
                
            logo_url = toolkit_data.get('logo')
            if not logo_url and 'meta' in toolkit_data and toolkit_data['meta']:
                if hasattr(toolkit_data['meta'], 'logo'):
                    logo_url = toolkit_data['meta'].logo
                elif isinstance(toolkit_data['meta'], dict):
                    logo_url = toolkit_data['meta'].get('logo')
            
            return logo_url if logo_url else None
            
        except Exception as e:
            logger.error(f"Error getting toolkit icon for {toolkit_slug}: {e}", exc_info=True)
            return None
