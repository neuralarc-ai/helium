...
async def discover_composio_tools(
    ...
    try:
        ...
        logger.error(f"HTTP error discovering tools for profile {profile_id}: {str(he.detail) if hasattr(he, 'detail') else str(he)}")
        raise
    except Exception as e:
        error_msg = f"Failed to discover tools for profile {profile_id}: {str(e)}"
