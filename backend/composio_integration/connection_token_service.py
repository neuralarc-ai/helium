...
class ConnectionTokenService:
    ...
    async def _get_session(self) -> httpx.AsyncClient:
        if self.session is None or self.session.is_closed:
            self.session = httpx.AsyncClient(
                ...
                headers={"User-Agent": "Suna-Composio-Client/1.0"}
            )
        return self.session
    ...
    async def create(self, external_user_id: ExternalUserId, app: Optional[AppSlug] = None) -> Dict[str, Any]:
        project_id = os.getenv("COMPOSIO_PROJECT_ID")
        environment = os.getenv("COMPOSIO_ENVIRONMENT", "development")
        
        if not project_id:
            raise AuthenticationError("Missing COMPOSIO_PROJECT_ID")
...
ComposioException = ConnectionTokenServiceError
