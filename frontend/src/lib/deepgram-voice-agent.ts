...
export class DeepgramVoiceAgent {
  ...
  private stopped = false; // Flag to check if conversation was stopped
  private currentAudio: HTMLAudioElement | null = null;
  private recognition: SpeechRecognition | null = null; // Add recognition instance property
  ...
  async stopConversation(): Promise<void> {
    ...
    // Stop all media streams (microphone access)
  ...
  /**
   * Stop all active media streams (microphone access)
   */
  private stopAllMediaStreams(): void {
    try {
      // Get all media streams from getUserMedia
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true })
          ...
          .catch(() => {
            // No active audio stream, which is fine
          });
      }
      
      // Also try to enumerate and stop any existing streams
      if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
        navigator.mediaDevices.enumerateDevices()
          .then(devices => {
            devices.forEach(device => {
              if (device.kind === 'audioinput') {
                console.log('Audio input device found:', device.label);
              }
            });
          })
          .catch(error => {
  ...
  /**
   * Reset the voice agent to initial state
   */
  reset(): void {
    console.log('Resetting voice agent...');
    this.stopped = false;
    this.isRecording = false;
    this.isSpeaking = false;
    this.recognition = null;
    this.currentAudio = null;
    this.websocket = null;
    console.log('Voice agent reset complete');
  }

  /**
   * Check if the voice agent is completely stopped
   */
  isStopped(): boolean {
    return this.stopped && !this.isRecording && !this.isSpeaking;
  }

  /**
   * Check if the voice agent is completely inactive (no ongoing activities)
   */
  isInactive(): boolean {
    return !this.isRecording && !this.isSpeaking && !this.recognition;
  }
  ...
  private async startListening(onTranscript: (text: string) => void, onResponse: (text: string) => void): Promise<void> {
    ...
    const startRecognition = () => {
      try {
        // Create new recognition instance and store it in class property
